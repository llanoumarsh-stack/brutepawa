...
const LIFE_EVENT_CATEGORIES = [
  { id: "travail",       icon: "💼", label: "Nouvel emploi / promotion",     color: "#1877F2" },
  { id: "etudes",        icon: "🎓", label: "Diplôme / études",              color: "#7B1FA2" },
  { id: "relation",      icon: "❤️", label: "Relation amoureuse",            color: "#E91E63" },
  { id: "fiancailles",   icon: "💍", label: "Fiançailles",                   color: "#F57C00" },
  { id: "mariage",       icon: "💒", label: "Mariage",                       color: "#E53935" },
  ...
  { id: "entreprise",    icon: "🚀", label: "Création d'entreprise",         color: "#F57C00" },
...
export default function CreateLifeEventPage() {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 50, background: "var(--fb-bg)", display: "flex", flexDirection: "column" }}>
      ...
      {/* Header */}
      <div style={{ background: "#fff", padding: "12px 16px", display: "flex", alignItems: "center", gap: 12, borderBottom: "1px solid #e4e6eb", flexShrink: 0 }}>
        <button onClick={() => navigate("/")} style={{ background: "none", border: "none", fontSize: 22, cursor: "pointer", color: "#1877F2", padding: "4px 8px" }}>←</button>
        <div style={{ flex: 1, fontWeight: 800, fontSize: 17 }}>Créer un évènement marquant</div>
        <button
          onClick={handlePublish}
          disabled={!selected || submitting}
          style={{
            background: selected && !submitting ? "#1877F2" : "#e4e6eb",
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "16px" }}>
        {error && <div style={{ background: "#ffebee", color: "#b00020", borderRadius: 10, padding: "10px 14px", marginBottom: 14, fontSize: 13 }}>{error}</div>}
        ...
        {selectedCat && (
          <div style={{ background: `${selectedCat.color}18`, border: `1.5px solid ${selectedCat.color}40`, borderRadius: 14, padding: "16px 18px", marginBottom: 16, display: "flex", alignItems: "center", gap: 14 }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontWeight: 800, fontSize: 15, color: "#050505" }}>{selectedCat.label}</div>
              <div style={{ fontSize: 12, color: "#65676b", marginTop: 2 }}>Catégorie sélectionnée</div>
            </div>
            <button onClick={() => setSelected(null)} style={{ background: "none", border: "none", fontSize: 20, cursor: "pointer", color: "#65676b" }}>✕</button>
          </div>
        )}

        {/* Detail input */}
        <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #e4e6eb", padding: "14px 16px", marginBottom: 20 }}>
          <textarea
            placeholder={selectedCat ? `Raconte cet évènement — ${selectedCat.label.toLowerCase()}…` : "Décris ton évènement marquant…"}
            value={detail}
            onChange={e => setDetail(e.target.value)}
            style={{ width: "100%", border: "none", outline: "none", fontFamily: "inherit", fontSize: 15, color: "#050505", resize: "none", minHeight: 70, boxSizing: "border-box" }}
            maxLength={500}
          />
        ...
        {/* Category grid */}
        <div style={{ fontWeight: 800, fontSize: 15, marginBottom: 12, color: "#050505" }}>Choisir une catégorie</div>
          ...
          {LIFE_EVENT_CATEGORIES.map(cat => (
            <button
              ...
              style={{
                ...
                border: selected === cat.id ? `2px solid ${cat.color}` : "1.5px solid #e4e6eb",
            ...
            >
              ...
              <div style={{ fontSize: 12, fontWeight: 700, color: selected === cat.id ? cat.color : "#050505", textAlign: "center", lineHeight: 1.3 }}>{cat.label}</div>
            </button>
          ))}
        </div>
      </div>
