...
const AVATAR_COLORS = ["#1877F2","#E91E63","#9C27B0","#F57C00","#388E3C","#00838F"];
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 500, background: "#F0F2F5", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 16px 8px", background: "#fff", borderBottom: "1px solid #E4E6EB" }}>
        <span style={{ fontSize: 13, fontWeight: 600, color: "#050505", display: "flex", alignItems: "center", gap: 4 }}>
          Mode payant
          <span style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", width: 16, height: 16, borderRadius: "50%", background: "#65676B", color: "#fff", fontSize: 10, fontWeight: 700 }}>?</span>
        </span>
      ...
      <div style={{ display: "flex", alignItems: "center", padding: "12px 16px", background: "#fff", borderBottom: "1px solid #E4E6EB" }}>
        ...
        <button style={{ background: "none", border: "none", cursor: "pointer", padding: 0 }}>
          <div style={{ width: 32, height: 32, borderRadius: "50%", background: "#E4E6EB", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18 }}>👤</div>
        </button>
      </div>

      <div style={{ flex: 1, overflowY: "auto" }}>
        {/* Avatar + name */}
        <div style={{ background: "#fff", padding: "20px 16px 16px", display: "flex", alignItems: "center", gap: 14, borderBottom: "1px solid #E4E6EB" }}>
          <div style={{ position: "relative", flexShrink: 0 }}>
            ...
            <div style={{ position: "absolute", bottom: 0, right: -2, width: 18, height: 18, borderRadius: "50%", background: "#4CAF50", border: "2px solid #fff", display: "flex", alignItems: "center", justifyContent: "center" }}>
        ...
        <div style={{ padding: "16px 16px 0" }}>
          ...
          <p style={{ margin: "0 0 14px", fontSize: 14, color: "#65676B" }}>
            Pour utiliser ces fonctionnalités, vous devez être éligible et éviter d'enfreindre les règles.
          </p>
          <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #E4E6EB", overflow: "hidden", marginBottom: 20 }}>
            {[
              { icon: "⭐", label: "Recommandations", status: "Actif", statusColor: "#050505" },
              { icon: "⚠️", label: "Monétisation", status: "Non éligible", statusColor: "#65676B" },
            ].map((row, i) => (
              <button key={row.label} style={{ width: "100%", background: "none", border: "none", padding: "14px 16px", display: "flex", alignItems: "center", gap: 14, cursor: "pointer", textAlign: "left", borderBottom: i === 0 ? "1px solid #F0F2F5" : "none" }}>
                <div style={{ width: 44, height: 44, borderRadius: 10, background: "#1C1E21", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, flexShrink: 0 }}>
                  {row.icon}
                </div>
                <div style={{ flex: 1 }}>
                ...
                <span style={{ color: "#65676B", fontSize: 20 }}>›</span>
              </button>
            ))}
          </div>
          ...
          <p style={{ margin: "0 0 16px", fontSize: 14, color: "#65676B", lineHeight: 1.5 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 20 }}>
            {[
              { title: "Présentation des Standards de la communauté", bg: "#E8F5E9", emoji: "👴" },
              { title: "Application des Standards de la communauté", bg: "#FCE4EC", emoji: "👩" },
            ].map(card => (
              <button key={card.title} style={{ background: "#fff", border: "1px solid #E4E6EB", borderRadius: 12, overflow: "hidden", textAlign: "left", cursor: "pointer", padding: 0 }}>
          ...
          <p style={{ margin: "0 0 12px", fontSize: 14, color: "#65676B", lineHeight: 1.5 }}>
          ...
          <button
            ...
            style={{ width: "100%", background: "#fff", border: "1.5px solid #E4E6EB", borderRadius: 8, padding: "12px 16px", fontSize: 14, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, marginBottom: 24 }}
          >
      ...
      {(survey === "rating" || survey === "text") && (
        <div style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,0.4)", zIndex: 10, display: "flex", alignItems: "flex-end" }}
          ...
          <div style={{ background: "#fff", borderRadius: "20px 20px 0 0", width: "100%", padding: "20px 20px 40px" }}
            ...
            <button onClick={() => setSurvey("none")} style={{ background: "none", border: "none", fontSize: 22, cursor: "pointer", marginBottom: 16, color: "#050505", padding: 0 }}>✕</button>
            ...
            {survey === "rating" && (
              <>
                ...
                <div style={{ border: "1.5px solid #E4E6EB", borderRadius: 12, overflow: "hidden", marginBottom: 20 }}>
                  {RATINGS.map((r, i) => (
                    <button key={r} onClick={() => setRating(r)} style={{ width: "100%", background: "none", border: "none", padding: "14px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", cursor: "pointer", borderBottom: i < RATINGS.length - 1 ? "1px solid #F0F2F5" : "none" }}>
                      <span style={{ fontSize: 14 }}>{r}</span>
                      <div style={{ width: 20, height: 20, borderRadius: "50%", border: `2px solid ${rating === r ? "#1877F2" : "#BCC0C4"}`, background: rating === r ? "#1877F2" : "#fff", display: "flex", alignItems: "center", justifyContent: "center" }}>
                        {rating === r && <div style={{ width: 8, height: 8, borderRadius: "50%", background: "#fff" }} />}
                      </div>
                    </button>
                  ))}
                </div>
                <button onClick={() => { if (rating) setSurvey("text"); }} style={{ width: "100%", background: rating ? "#1877F2" : "#BCC0C4", color: "#fff", border: "none", borderRadius: 24, padding: "14px", fontSize: 15, fontWeight: 700, cursor: rating ? "pointer" : "default" }}>
                  Continuer
                </button>
              </>
            )}
            ...
            {survey === "text" && (
              <>
                ...
                <input
                  ...
                  style={{ width: "100%", border: "1.5px solid #E4E6EB", borderRadius: 10, padding: "12px 14px", fontSize: 14, marginBottom: 16, boxSizing: "border-box", outline: "none" }}
                />
                <button onClick={() => setSurvey("done")} style={{ width: "100%", background: "#1877F2", color: "#fff", border: "none", borderRadius: 24, padding: "14px", fontSize: 15, fontWeight: 700, cursor: "pointer" }}>
                  Continuer
                </button>
              </>
            )}
      ...
      {survey === "done" && (
        <div style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,0.4)", zIndex: 10, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div style={{ background: "#fff", borderRadius: 16, padding: "32px 24px", margin: "0 24px", textAlign: "center" }}>
            ...
            <div style={{ fontSize: 14, color: "#65676B", marginBottom: 20 }}>Votre retour nous aide à améliorer Brute Pawa.</div>
            <button onClick={() => setSurvey("none")} style={{ background: "#1877F2", color: "#fff", border: "none", borderRadius: 8, padding: "12px 32px", fontSize: 15, fontWeight: 700, cursor: "pointer" }}>Fermer</button>
          </div>
        </div>
      )}
    </div>
