...
export default function FavoritesScreen() {
  ...
  const contactQuery = useQuery<{ isFavorite: boolean }>({
  ...
  const currentlyFav = contactQuery.data?.isFavorite ?? false;
  const [action, setAction] = useState<"add" | "remove">("add");

  React.useEffect(() => {
    setAction(currentlyFav ? "remove" : "add");
  }, [currentlyFav]);

  const mutation = useMutation({
    mutationFn: async () => {
      const method = action === "add" ? "POST" : "DELETE";
      const r = await fetch(`${API_BASE_URL}/contacts/${userId}/favorite`, {
        method,
  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      <View style={[styles.header, { paddingTop: topPadding, backgroundColor: colors.card, borderBottomColor: colors.border }]}>
        ...
        <Text style={[styles.headerTitle, { color: colors.foreground }]}>Ajouter aux favoris</Text>
        <View style={{ width: 34 }} />
      </View>

      <View style={styles.content}>
        <View style={[styles.iconWrap, { backgroundColor: colors.secondary }]}>
          <Ionicons name="star-outline" size={44} color={colors.primary} />
        </View>

        <Text style={[styles.title, { color: colors.foreground }]}>Ajouter aux favoris</Text>
        <Text style={[styles.desc, { color: colors.mutedForeground }]}>
          Retrouvez facilement ce contact dans vos favoris
        </Text>

        <View style={[styles.optionsCard, { backgroundColor: colors.card }]}>
          {[
            { key: "add" as const,    label: "Ajouter aux favoris"  },
            { key: "remove" as const, label: "Retirer des favoris"   },
          ].map((opt, i) => (
            <TouchableOpacity
              key={opt.key}
              style={[
